...
messagebox.showinfo(title="Alert message", message="This is just a alert message")

# creating a question to get the response from the user [Yes or No Question]
response = messagebox.askquestion(title="Simple question", message="Do you love Python?")

# If user clicks 'Yes' then it returns 1 else it returns 0
if response == 'yes':
    Label(window, text="You love Python").pack()
else:
